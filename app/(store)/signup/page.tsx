...
import { ArrowRight, Loader2, Lock, Mail, User as UserIcon, AlertCircle, ShieldCheck, Phone, CheckCircle, RefreshCw } from 'lucide-react';
...
function SignupContent() {
    ...
    const formatPhoneNumber = (raw: string) => {
        let clean = raw.trim().replace(/\s+/g, '').replace(/-/g, '');
        if (!clean.startsWith('+')) {
            if (clean.length === 10) {
                clean = '+91' + clean;
    ...
    const handleSignup = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.session && data.user) {
                ...
                setSuccessMessage('Account created! Please check your email to verify your registration, or sign in directly.');
            }
        } catch (err: any) {
    ...
    const handleSendPhoneOtp = async (e: React.FormEvent) => {
        ...
        try {
            const { error: otpError } = await supabase.auth.signInWithOtp({
                ...
                options: {
                    data: {
                        full_name: phoneName.trim() || `Customer ${fullPhone.slice(-4)}`,
                        name: phoneName.trim() || `Customer ${fullPhone.slice(-4)}`
                    }
                }
            });
            ...
            if (otpError) {
                if (otpError.message?.toLowerCase().includes('sms') || otpError.message?.toLowerCase().includes('provider')) {
                    throw new Error(`SMS Provider Notice: ${otpError.message}. Alternatively, you can use Google One-Click Registration or Email.`);
                }
                throw otpError;
            }
        ...
        try {
            ...
            if (data.session && data.user) {
                const userObj: User = {
                    ...
                    name: phoneName.trim() || data.user.user_metadata?.full_name || `Customer ${fullPhone.slice(-4)}`,
                    created_at: data.user.created_at
                };
                setUser(userObj);
    ...
    return (
        <div className="min-h-screen bg-[#151515] text-[#F8F3E8] flex items-center justify-center p-6 noise-overlay selection:bg-[#C9A84C]/30 relative overflow-hidden">
            ...
            >
                ...
                <div className="flex bg-[#151515] p-1.5 rounded-2xl border border-white/10 mb-8">
                    <button
                        type="button"
                        onClick={() => { setAuthMode('email'); setError(null); setSuccessMessage(null); }}
                        className={`flex-1 py-3 rounded-xl text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-2 ${
                            authMode === 'email'
                                ? 'bg-gradient-to-r from-[#E8D48B] to-[#C9A84C] text-[#0A0A0F] shadow-lg'
                                : 'text-[#86868B] hover:text-[#F8F3E8]'
                        }`}
                    >
                        <Mail size={15} />
                    ...
                    <button
                        type="button"
                        onClick={() => { setAuthMode('phone'); setError(null); setSuccessMessage(null); }}
                        className={`flex-1 py-3 rounded-xl text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-2 ${
                            authMode === 'phone'
                                ? 'bg-gradient-to-r from-[#E8D48B] to-[#C9A84C] text-[#0A0A0F] shadow-lg'
                                : 'text-[#86868B] hover:text-[#F8F3E8]'
                ...
                {authMode === 'email' ? (
                    <>
                        ...
                        <div className="relative flex justify-center text-[9px] font-black uppercase tracking-[0.3em] text-[#86868B] my-8">
                            ...
                            <span className="bg-[#1E1E1E] px-4 relative z-10">Or create credentials</span>
                        </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative group">
                                    ...
                                    <input
                                        ...
                                        className="w-full h-16 bg-[#151515] border border-white/10 rounded-2xl pl-16 pr-6 text-[#F8F3E8] placeholder-[#86868B] focus:border-[#C9A84C] focus:outline-none transition-all font-medium uppercase text-xs tracking-wider"
                    ...
                    /* Phone Number OTP Registration Mode */
                    <div className="space-y-6 relative z-10">
                        {!otpSent ? (
                            <form onSubmit={handleSendPhoneOtp} className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-[#86868B] ml-2">Mobile Phone Number</label>
                                    <div className="relative group flex items-center">
                                        <div className="absolute left-5 flex items-center gap-2 text-xs font-mono font-bold text-[#C9A84C] pointer-events-none">
                                            <Phone size={16} />
                                            <span>+91</span>
                                        </div>
                                        <input
                                            ...
                                            className="w-full h-16 bg-[#151515] border border-white/10 rounded-2xl pl-20 pr-6 text-[#F8F3E8] placeholder-[#86868B] focus:border-[#C9A84C] focus:outline-none transition-all font-mono font-bold text-sm tracking-widest"
                                            placeholder="98765 43210"
                                        />
                                    </div>
                                    <p className="text-[10px] text-[#86868B] ml-2 font-medium">We will send a 6-digit verification code to this phone number.</p>
                                </div>
                                ...
                                >
                                    {loading ? (
                                        ...
                                        <div className="flex items-center justify-center gap-3">
                                            Register with Phone <ArrowRight size={18} />
                                        </div>
                                    )}
                                </button>
                            </form>
                            ...
                            <form onSubmit={handleVerifyPhoneOtp} className="space-y-6">
                                <div className="space-y-2">
                                    <div className="flex justify-between items-center px-2">
                                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-[#86868B]">Enter 6-Digit OTP</label>
                                        <button
                                            type="button"
                                            onClick={() => setOtpSent(false)}
                                            className="text-[10px] font-bold text-[#C9A84C] uppercase tracking-wider hover:underline"
                                        >
                                            Change Phone
                                        </button>
                                    </div>
                                    <input
                                        ...
                                        className="w-full h-16 bg-[#151515] border border-white/10 rounded-2xl px-6 text-[#F8F3E8] placeholder-[#86868B] focus:border-[#C9A84C] focus:outline-none transition-all font-mono font-black text-xl tracking-[0.4em] text-center"
                                        placeholder="••••••"
                                    />
                                </div>
                                ...
                                <div className="text-center pt-2">
                                    {resendCooldown > 0 ? (
                                        ...
                                        <button
                                            ...
                                            className="text-[10px] font-bold text-[#C9A84C] uppercase tracking-wider hover:underline flex items-center justify-center gap-1.5 mx-auto"
                                        >
